# build_features2.py - spectrograms for the v2 sets only; v1's positive features and the 30 GB of
# general negatives are reused untouched. Same augmentation as build_features.py, so the two models
# are comparable. (Do NOT `from build_features import build`: that file runs its builds on import.)
import os, subprocess, sys
# the image ships a newer `datasets` whose audio decoding wants torchcodec; pin what train.sh pins
subprocess.run([sys.executable, "-m", "pip", "install", "--quiet", "--no-cache-dir", "datasets==2.19.0", "fsspec<=2024.3.1",
                "audiomentations", "mmap_ninja", "pymicro-features", "soundfile", "librosa"], check=False)
...
if __name__ == "__main__":
    build('/w/samples/positive_tless_flat', '/w/features/positive_tless')
    build('/w/samples/adversarial_flat2',   '/w/features/adversarial2')
    print("\nFEATURES2-DONE", flush=True)
